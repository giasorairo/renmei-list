...
import { useFontFamily, fontFamilies } from "./_hooks/use-font-family";
...
export default function Home() {
  const { names, setNamesEnhance, handleAddNames, handleDeleteNames } =
    useNames();
  const { fontFamily, handleSelectFontFamily } = useFontFamily();
  ...
  const handleClickEditCompanyModalOkButton = useCallback(
    (value: string) => {
      changeCompany(value);
  ...
  const handleClickEditDepartmentModalOkButton = useCallback(
    (value: string) => {
      ...
      <div className={styles.renmeiList}>
        <div className={styles.renmeiList__controller}>
          <div className={styles.renmeiList__buttonContainer}>
            <select
              className={styles.renmeiList__select}
              onChange={handleSelectFontFamily}
            >
              {fontFamilies.map((_fontFamily) => (
                <option key={_fontFamily.name} value={_fontFamily.name}>
                  {_fontFamily.displayName}
                </option>
              ))}
            </select>
            <Button color="black" onClick={handleAddNames}>
              名前を追加する
            </Button>
            <Button color="outlineBlack" onClick={handleDeleteNames}>
              名前を削除する
            </Button>
            <div className={styles.renmeiList__printButtonContainer}>
              <Button color="blue" onClick={handleClickPrint}>
                印刷する
              </Button>
            </div>
          </div>
